...
QGIS_BUILD_PATH=${QGIS_BUILD_PATH:-qgis_mapserv.fcgi}

echo "Running test $1 ..."
OUTPUT=$(QGIS_TEST_MODULE=${TEST_NAME} unbuffer ${QGIS_BUILD_PATH} --version-migration --nologo "$TEST_NAME"  2>/dev/null | tee /dev/tty)
